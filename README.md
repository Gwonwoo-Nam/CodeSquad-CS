...
- 단계 별 실행 파일은 최종 리비전에서 각각 Application_1,2,3,4를 실행하면 확인 할 수 있습니다.
- 단계 별 구현 요구 사항이 조금씩 달라 이를 각각 만족시키려다 보니 비슷한 기능을 하는 클래스가 여러 개 있습니다.(SolarCircle, Planets, PlanetsGUI 등)
...
- 콘솔 애니메이션 구현 완료 커밋 ID : [38e68d4]
  - 콘솔 구현 시 콘솔 애니메이션이 부드럽게 전환되지 않아 GUI 기반 변경
- JFrame과 Swing 등 API를 이용해서 애니메이션 구현
  - 더블 버퍼링 적용하여 이미지를 그리는 시간 동안 깜빡이는 현상 개선

#### 기능 구현 목록
- GUI 애니메이션 구현
- 프레임 당 시간 조절 기능 구현
  - 사용자에게 프레임 당 배속을 입력받는다.
    - 유효하지 않은 입력 발생 시 에러 메시지를 출력한다.
- 현재 시간을 표시하는 기능 구현
